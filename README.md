...
If you need to backup a generic postgresql and forward it to bu server with specific user [--bu_user] and specific ssh key for that user [--bu_key] to a directory [--bu_dir] that target user have access to that, try command below:  
```
...
If you are after backing up thingsboard's postgresql and forward it to a separqte backup server(bu), when you are runing thingsboard with docker do below  
```
